...
__kernel void calcCas(__global float * cas, __private size_t width, __private float twoPiN) {
    const int2 pos = {get_global_id(0), get_global_id(1)};
    cas[pos.y * width + pos.x] = sin(pos.x * pos.y * twoPiN) + cos(pos.x * pos.y * twoPiN);
}

__kernel void calcTrig(__global float * cosTable, __global float * sinTable, __private float coeff) {
    const int pos = get_global_id(0);
    const float angle = M_PI_F / 180.0 * pos;

    cosTable[pos] = cos(angle * coeff);
    sinTable[pos] = sin(angle * coeff);
}
